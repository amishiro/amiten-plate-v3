...
{
  $url = isDev($entry) ? 'http://localhost:3000/' . $entry : assetUrl($entry);
...
{
  if (isDev($entry) || !importsUrls($entry)) {
    return '';
  }
...
function assetUrl(string $entry): string
{
  ...
  return isset($manifest[$entry]) ? BASE_URL . $manifest[$entry]['file'] : '';
}
